...
import com.moko.commuregw.databinding.ActivityDismissSosAlarmNotifyBinding;
...
public class DismissSOSAlarmNotifyActivity extends BaseActivity<ActivityDismissSosAlarmNotifyBinding> {
    ...
    protected void onCreate() {
        ...
        getSOSAlarmNotify(MQTTConstants.CONFIG_MSG_ID_BLE_BXP_BUTTON_GET_DISMISS_SOS_ALARM);
    }

    @Override
    protected ActivityDismissSosAlarmNotifyBinding getViewBinding() {
        return ActivityDismissSosAlarmNotifyBinding.inflate(getLayoutInflater());
    }
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (msg_id == MQTTConstants.NOTIFY_MSG_ID_BLE_BXP_BUTTON_GET_DISMISS_SOS_ALARM) {
            ...
            mBind.etLedDuration.setText(String.valueOf(ledDuration / 100));
        ...
        if (msg_id == MQTTConstants.NOTIFY_MSG_ID_BLE_BXP_BUTTON_SET_DISMISS_SOS_ALARM) {
            Type type = new TypeToken<MsgNotify<JsonObject>>() {
            }.getType();
            MsgNotify<JsonObject> result = new Gson().fromJson(message, type);
            if (!mMokoDevice.mac.equalsIgnoreCase(result.device_info.mac))
    ...
    private void setSOSAlarmNotify(int ledInterval, int ledDuration, int buzzerInterval, int buzzerDuration) {
        ...
        int msgId = MQTTConstants.CONFIG_MSG_ID_BLE_BXP_BUTTON_SET_DISMISS_SOS_ALARM;
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("mac", mBXPButtonInfo.mac);
        jsonObject.addProperty("led_color", selectedStr);
        jsonObject.addProperty("led_off_time", ledInterval * 100);
        jsonObject.addProperty("led_work_time", ledDuration * 100);
